...
**Status:** ⬜ Not started · **Depends on:** SPEC-04 (finals via `SessionController.ingestFinal`),
SPEC-05 (Active Session layout), SPEC-01 (config) · **Extends:** SPEC.md §9 (UI) & §12 (config)
— this is a **new feature**, not in SPEC.md v2.0.

> **Progress:** nothing built. This spec adds a second, on-device model — a small **local LLM
> (MLX)** — that watches the live transcript and, **every ~100 words**, emits a short "what the
> other person is actually asking, in easy English" card in a panel to the **right** of the live
> captions.
...
- **Content-paced, not clock-paced.** A new summary fires each time ~100 words of speech
...
- **Trigger = every 100 words** of committed transcript (not a time cadence). When the running
  word count of finals reaches ≥ `words_per_summary` (default **100**), summarize that block, emit
...
While recording, keep a scrolling "Key points" panel to the right of the captions. Every ~100
...
- [ ] `Summary/SummaryEngine.swift` — a **background `actor`** (MLX inference must not run on the
  ...
  - [ ] `summarize(chunk:) async -> SummaryResult` — one generation per ~100-word block, greedy
...
- [ ] `SummaryPrompt.swift` — build the system+user prompt from a **single ~100-word chunk** (no
      prior summary):
...
- [ ] When the running count **reaches ≥ `words_per_summary`** (default 100): capture the buffer,
      **reset buffer + counter to zero**, and dispatch `SummaryEngine.summarize(chunk:)`. Do **not**
      split a final mid-way — the block is "≥100 words" (the final that crosses the line is
      included whole).
...
- [ ] **Overlap safety:** if a block completes while the previous generation is still running,
      **queue at most one** pending block; if a second arrives, merge it into the queued one (never
      build an unbounded backlog). With ~100 words ≈ ~40 s of speech vs a ~3–4 s generation, this is
      rare, but must be handled.
- [ ] Freeze with the clock: no new summaries while `paused`; the counter resumes on `resume()`.
- [ ] On `pause()`/`stop()`: **flush** — summarize the remaining `<100`-word buffer as a final card
...
ingestFinal(text) ─► buffer += text ; words += wordCount(text)   (SessionController, @MainActor)
        │
   words ≥ 100 ? ─► capture block ; reset buffer + counter to 0
        │ yes
...
pause()/stop() ─► flush remaining <100-word buffer as a final card
```
...
- **No cross-block memory** — each ~100-word card is independent (a pronoun referring back >100
...
- **B7 — Latency vs. trigger (downgraded).** ~100 words ≈ ~40 s of speech ≫ ~3–4 s generation, so
...
- With a session recording, each **~100 words** of speech adds one **≤5-line, simple-English** card
  to the "Key points" list; the counter resets and counts the next 100. Existing cards never flicker.
...
- Pausing/stopping flushes a final card for the remaining `<100`-word tail; no summaries while paused;
...
4. **Words-per-summary default (100).** 100 words ≈ ~40 s of normal speech → a card every ~40 s in
